...
# you may not use this file except in io.personiumpliance with the License.
...
ROOT_DIR=
DATE=`/bin/date +%Y%m%d`
SWEEP_LOG=${ROOT_DIR}/personium/personium-cell-sweeper/log/personium-cell-sweeper.log
SWEEP_LOG_OLD=${SWEEP_LOG}.`/bin/date +%Y%m%d -d '1 days ago'`
LOCK_FILE=${ROOT_DIR}/personium/personium-cell-sweeper/personium-cell-sweeper.lock
...
function read_properties() {
  ...
  MYSQL_USER=`/bin/grep 'io.personium.fujitsu.dc.core.mysql.master.user.name' $1 | grep -v '^[:blank:]*#'|  /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.fujitsu.dc.core.mysql.master.user.name' is not defined."
    abort 1
  fi 
  ...
  MYSQL_PASS=`/bin/grep 'io.personium.fujitsu.dc.core.mysql.master.user.password' $1 | grep -v '^[:blank:]*#' | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.fujitsu.dc.core.mysql.master.user.password' is not defined."
    abort 1
  fi 
  ...
  MYSQL_HOST=`/bin/grep 'io.personium.fujitsu.dc.core.mysql.master.host' $1 | grep -v '^[:blank:]*#' | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.fujitsu.dc.core.mysql.master.host' is not defined."
    abort 1
  fi 
