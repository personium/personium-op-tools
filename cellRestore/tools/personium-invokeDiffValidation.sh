...
#   DavFsMiss dav_id:[F8RXW9qRTZupbC7-ge-1pB], file_path:[/personiumnfs/dc-core/dav/vet/F8/RX/F8RXW9qRTZupbC7-ge-1pB]
#
# 共通関数定義ファイル読み込み
...
# 引数チェック
if [ $# -lt 2 ]; then
  outputErrorLog "Usage: personium-invokeDiffValidation.sh <unituser-name> <cell-id> <work-dir>"
  exit 3
fi
...
# 利用するプロパティ
PROP_ES_MASTER_HOST="io.personium.core.es.master.host"
PROP_UNIT_PREFIX="io.personium.core.es.unitPrefix"
PROP_MY_SQL_HOST="io.personium.core.mysql.slave.host"
PROP_MY_SQL_PORT="io.personium.core.mysql.slave.port"
PROP_MY_SQL_USER="io.personium.core.mysql.backup.user.name"
PROP_MY_SQL_PASSWORD="io.personium.core.mysql.backup.user.password"
PROP_DAV_PATH="io.personium.core.webdav.basedir"
...
# Cell以外
python ${TOOL_DIR}/personium-searchCellDataListFromElasticsearch.py ${ES_MASTER_HOST} ${UNIT_PREFIX} ${UNIT_USER} ${CELL_ID} "${WORK_DIR}/es.txt"
RET_CODE=${?}
if [ ${RET_CODE} -ne 0 ]; then
