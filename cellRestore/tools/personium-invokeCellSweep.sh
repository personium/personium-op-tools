...
CELL_SWEEP_DIR=/personium/personium-cell-sweeper

# ロックファイルが存在するか確認し、存在しない場合は作成する
## このシェルスクリプトは、cell sweepのロックが存在しない前提として実行する
## ロックファイルが存在する場合、ログを出力し異常終了とする
ROOT_DIR=
LOCK_FILE=${ROOT_DIR}${CELL_SWEEP_DIR}/personium-cell-sweeper.lock
if [ -f ${LOCK_FILE} ]; then
...
if [ $? -ne 0 ]; then
  outputErrorLog "Failed to sweep target cell in PCS. (Cause: see. ${CELL_SWEEP_DIR}/log/personium-cell-sweeper.log) Process aborted."
  exit 2
fi
