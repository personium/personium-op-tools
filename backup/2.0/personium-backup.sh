...
ROOT_DIR=
DATE=`/bin/date +%Y%m%d`
BACKUP_LOG=${ROOT_DIR}/personium/personium-backup/log/personium_backup.log
BACKUP_LOG_OLD=${BACKUP_LOG}.`/bin/date +%Y%m%d -d '1 days ago'`
LOCK_FILE=${ROOT_DIR}/personium/personium-backup/personium-backup.lock

# MySQL関連
MYSQL_SRC_DIR=${ROOT_DIR}/personium/mysql/data
MYSQL_WORK_DIR=${ROOT_DIR}/personium/mysql/data_backup.tmp
MYSQL_DST_DIR=${ROOT_DIR}/personium/mysql/data_${DATE}
...
function read_properties() {
  ...
  UNIT_PREFIX=`/bin/grep 'io.personium.core.es.unitPrefix' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.es.unitPrefix' is not defined."
    abort 1
  fi 
  ...
  MYSQL_USER=`/bin/grep 'io.personium.core.mysql.backup.user.name' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.mysql.backup.user.name' is not defined."
    abort 1
  fi 
  ...
  MYSQL_PASS=`/bin/grep 'io.personium.core.mysql.backup.user.password' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.mysql.backup.user.password' is not defined."
    abort 1
  fi 
  ...
  MYSQL_HOST=`/bin/grep 'io.personium.core.mysql.slave.host' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.mysql.slave.host' is not defined."
    abort 1
  fi 
  ...
  MYSQL_PORT=`/bin/grep 'io.personium.core.mysql.slave.port' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.mysql.slave.port' is not defined."
    abort 1
  fi 
  ...
  BACKUP_GENERATION=`/bin/grep 'io.personium.core.backup.count' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.backup.count' is not defined."
    abort 1
  fi 
  ...
  SOPOS_SPLIT_SIZE=`/bin/grep 'io.personium.core.backup.splitSize' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.backup.splitSize' is not defined."
    abort 1
  fi 
  ...
  # MySQLバックアップの圧縮オプション
  ARCHIVE_ENABLE=`/bin/grep 'io.personium.core.backup.mysql.archive.enable' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
  ...
  BACKUP_SRC_DAV_ROOT=`/bin/grep 'io.personium.core.webdav.basedir' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.webdav.basedir' is not defined."
    abort 1
  fi 
  ...
  BACKUP_DEST_DAV_ROOT=`/bin/grep 'io.personium.core.backup.dav.dir' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.backup.dav.dir' is not defined."
    abort 1
  fi 
  ...
  BACKUP_SRC_EVLOG_ROOT=`/bin/grep 'io.personium.core.eventlog.basedir' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.eventlog.basedir' is not defined."
    abort 1
  fi 
  ...
  BACKUP_DEST_EVLOG_ROOT=`/bin/grep 'io.personium.core.backup.eventlog.dir' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ $? -ne 0 ]; then
    outputWarnLog "property 'io.personium.core.backup.eventlog.dir' is not defined."
    abort 1
  fi 
...
# -pオプションで渡されたpersonium-unit-config.propertiesファイルから必要なプロパティを読み込む
