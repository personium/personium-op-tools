...
ROOT_DIR=
DATE=`/bin/date +%Y%m%d`
BACKUP_LOG=${ROOT_DIR}/personium/personium-backup/log/personium-backup.log
...
function readProperties() {
  ...
  # MySQLのユーザID
  MYSQL_USER=`/bin/grep 'io.personium.core.mysql.master.user.name' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ -z ${MYSQL_USER} ]; then
    outputWarnLog "property 'io.personium.core.mysql.master.user.name' is not defined."
    abort 1
  fi 

  # MySQLのパスワード
  MYSQL_PASS=`/bin/grep 'io.personium.core.mysql.master.user.password' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ -z ${MYSQL_PASS} ]; then
    outputWarnLog "property 'io.personium.core.mysql.master.user.password' is not defined."
    abort 1
  fi

  # MySQLのホスト
  MYSQL_HOST=`/bin/grep 'io.personium.core.mysql.master.host' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ -z ${MYSQL_HOST} ]; then
    outputWarnLog "property 'io.personium.core.mysql.master.host' is not defined."
    abort 1
  fi

  # MySQLのポート
  MYSQL_PORT=`/bin/grep 'io.personium.core.mysql.master.port' $1 | /bin/sed -e 's/^.*=//g' | /bin/sed 's/\r//'`
  if [ -z ${MYSQL_PORT} ]; then
    outputWarnLog "property 'io.personium.core.mysql.master.port' is not defined."
    abort 1
  fi
