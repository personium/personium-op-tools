...
#
#
# personium-readPropertyFile.sh - CONF_PATH変数で渡されたプロパティファイルを読み込む
...
function checkMandatoryProperties() {
  ...
  if [ -z "${val}" ]; then 
    outputErrorLog "Mandatory property '${target}' is not specified in configuration file 'personium/personium-unit-config.properties'".
    returnValue=1
  fi      
  done
  return ${returnValue}
...
function outputInfoLog() {
  echo "`/bin/date +'%Y/%m/%d %H:%M:%S'` [INFO ] ${1}" >> ${MX_LOG}  2>&1
}

# WARNログ出力
# 引数：メッセージ
function outputWarnLog() {
  echo "`/bin/date +'%Y/%m/%d %H:%M:%S'` [WARN ] ${1}" >> ${MX_LOG}  2>&1
}

# ERRORログ出力
# 引数：メッセージ
function outputErrorLog() {
  echo "`/bin/date +'%Y/%m/%d %H:%M:%S'` [ERROR] ${1}" >> ${MX_LOG}  2>&1
}
