...
FULL_GC_LOG_DIRECTORY=/fj/elasticsearch/log
...
do
        ...
        if [ -n "$ES_PROCESS_ID" ]; then
                ...
                /opt/jdk/bin/jstat -gcutil -t $ES_PROCESS_ID 5s | /bin/awk '{print strftime("%Y/%m/%d %H:%M:%S"), $0; fflush()}' | /bin/awk -f /fj/dc-esgclog/printFullGC.awk >> $FULL_GC_LOG_DIRECTORY/$FULL_GC_LOG_FILE_NAME
